...
class TestVehicleControlResp(unittest.TestCase):
    def test_rvc_req_sts_decoded(self) -> None:
        ...
        assert sut.rvc_req_sts_decoded == b"\x01"

    def test_rvc_req_type_decoded(self) -> None:
        sut = VehicleControlResp(
            rvcReqType="Bg==",
        )
        assert sut.rvc_req_type_decoded == b"\x06"
    ...
    def test_decode_from_json(self) -> None:
        ...
        assert decoded.basicVehicleStatus is not None
        assert decoded.gpsPosition is not None
        assert decoded.rvcReqType == "Bg=="
        assert decoded.rvc_req_type_decoded == b"\x06"
        assert decoded.rvcReqSts is not None, "AQ=="
        assert decoded.rvc_req_sts_decoded == b"\x01"
